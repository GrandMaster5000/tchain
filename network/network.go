...
const (
	// Гарантирует завершенность данных
...
type Conn net.Conn
type Handle func(Conn, *Package)

func Handler(option int, conn Conn, pack *Package, handle func(*Package) string) bool {
...
func Listen(address string, handle Handle) Listener {
...
func serve(listener Listener, handle Handle) {
...
func handleConn(conn Conn, handle Handle) {
	defer conn.Close()
